...
\subsection{Crash course in algebraic number theory}\label{sec:crash-course}
...
$\# I_\fp = e_\fp$, whence $\fp$ is unramified in $L$ if and only if 
$I_\fp = 1$. 
...
In other words, we require the existence of a smooth proper scheme $\cX$ over 
$\fo_\fp$ such that the following diagram is cartesian:
\[\xymatrix{
  X \ar[r] \ar[d]  
    & \cX \ar[d] \\
  \spec(k_\fp) \ar[r] 
...
the structure of an abelian scheme over $\fo_k$, and that $A_\fp$ is an abelian 
variety over $\kappa_\fp$. 
...
models always exist. Since the functor $X\mapsto \hom_k(X_k,A)$ that 
$\cA$ represents is naturally group-valued, $\cA$ has the structure of a 
commutative group scheme \cite[1.2.6]{blr90}. 

So if $S=\spec(\fo_k)$ for a number field $k$ and $\fp\subset \fo$ is a prime, 
we could have said that an abelian variety $A$ over $k$ has good reduction at 
$\fp$ if the N\'eron model $\cA$ for $\fo_\fp$ (the localization of $\fo$ at 
$\fp$) is proper as an $\fo_\fp$-scheme. 
...
homomorphism. It is given as the composite 
\[
...
Let $S$ be the finite set of primes for which $A$ has bad reduction; we will 
show that the group
\[
  \h_S^1(k,\torsion n A) = \left\{[\varphi]\in \h^1(k,\torsion n A) : \varphi(I_\fp) = 0\text{ for all } \fp\notin S\right\}
\]
is finite. 
...
ramification'' 
\[
  \h_S^1(G_k,M) = \left\{[\varphi]\in \h^1(G,M) : \varphi(I_\fp) = 0\text{ for all }\fp\notin S\right\} .
\]
...
$\pi_1(\spec k)$, the group $G_{k,S}$ is an \'etale fundamental group via the 
following result.
...
For all but finitely many $\fp\subset \fo$, the variety $A$ has good reduction 
at $\fp$; choose one such prime. We have a reduction map 
...
subgroup of $A_\fp(\kappa_\fp)\times A_{\fp'}(\kappa_{\fp'})$. Incidentally, 
This gives us a 
way to compute $A(k)_\text{tors}$, because $A_\fp(\kappa_\fp)$ and 
$A_{\fp'}(\kappa_{\fp'})$ are computable. Indeed, the set $X(\kappa)$ for $X$ 
any projective variety over any finite field $\kappa$ is computable for 
trivial reasons (everything involved is finite!).
...
\ref{sec:res-ram} acts on $\torsion n A$, so we can consider the cohomology group 
...
\begin{enumerate}
  \item $\torsion n A(k^s)$ is finite, 
  \item $G_{k,S}$ is small, and 
  \item \autoref{thm:small-groups}. 
\end{enumerate}
...
\]
The \emph{Weil-Ch\^atelet group} of $A$, written $\weilchatelet(A)$, is 
the group of torsors over $A$ modulo equivalence. As we have seen, there is a 
bijection $\weilchatelet(A) = \h^1(k,A)$. If $k$ is a number field, then 
$\weilchatelet(A)$ is infinite, but if $k$ is finite then 
$\weilchatelet(A)=0$. We'll define the map 
$\weilchatelet(A)\to \h^1(k,A)$. Fix an $A$-torsor $X$. There exists some 
...
$\weilchatelet(A)$ the structure of an abelian group -- alternatively one 
...
$H_\dQ\left(\frac 1 2:1\right)=2$, while 
$H_\dQ\left(\frac{100001}{200001}:1\right)=200001$, which is much larger. 

Let $k$ be a number field. Each finite place $v$ of $k$ is associated with 
...
because it doesn't satisfy the triangle inequality. The following result, 
briefly touched on in \ref{sec:crash-course}, is known as the \emph{product 
formula}. 
...
\[
  H_k(a_0:\cdots:a_n) = \prod_v \sup\left\{\|a_0\|_v,\dots,\|a_n\|_v\right\} .
\]
...
\begin{enumerate}
  \item (Normalization). If $D$ is very ample, then $h_D=h\circ \phi_D+O(1)$. 
  \item (Functoriality). For a morphism $\phi:X\to Y$ of nice $k$-varieties, we 
    have $h_{X,\phi^\ast D} = h_{Y,D}\circ\phi + O(1)$ for all $D\in \divisor(Y)$. 
  \item (Positivity). For all effective $D$, $h_D\geqslant O(1)$ on the 
    complement of the base locus of $D$. 
\end{enumerate}
\end{theorem}
\begin{proof}
...
\begin{align*}
  \widehat h_c(x+y+z) &-\widehat h_c(x+y)-\widehat h_c(z) \\
    &= \left(\widehat h_c(x+z)-\widehat h_c(x)-\widehat h_c(z)\right) + \left(\widehat h_c(y+z)-\widehat h_c(y)-\widehat h_c(z)\right) \text{.}
\end{align*}
...
\begin{theorem}
Let $c$ be the class of a very ample line bundle. Then 
...
$\dN \cdot x\subset \{a\in A(L):\widehat h_c(a)\leqslant 0\}$, which we 
