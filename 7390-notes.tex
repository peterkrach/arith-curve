...
$x_1 = y_{i_1} + n x_2$, and one easily shows that 
\[
  |x_2| \leqslant \frac{|y_{i_1}| + |x_1|}{n} 
    \leqslant \frac{c}{2 n} + \frac 1 n |x_1|
\]
Setting $x_2 = y_{i_2} + n x_3$, we can repeat this process, obtaining the 
inequality 
\[
  |x_{i+1}| \leqslant \frac 1 n |x_i| + \frac{c}{2 n}
    \leqslant \frac{1}{n^i} |x_1| + \frac 1 2\left(\frac 1 n + \cdots + \frac{1}{n^i}\right) c 
    \leqslant \frac{|x_1|}{n^i} + \frac c 2
\]
For $i\gg 0$, the quantity on the right is less than $c$. Thus $x_1$ is in the 
...
absolute value associated with $v$. For $x\in A$, let 
...
height} associated with $c$. It yields a homomorphism of abelian groups 
$\widehat h:\pic(A)\to \dR^{A(\bar k)}$, and if we let 
$\operatorname{R}_k(A) = \mathbb{R}^{A(\bar k)}$, then the N\'eron-Tate height 
is even a natural transformation $\widetilde h : \pic\to \operatorname{R}_k$. 
The function $|\cdot|$ used in the proof of is just $\widehat h_c^{1/2}$ for 
some very ample even divisor $c$. For more details, see \cite[9.2]{bg06}.
