...
\cite{?:cf86}. In any case, to give an element of $\brauer(k)$, it is sufficient 
to  give an element of $\brauer(k_{v_i})=\dQ/\dZ$ for finitely many finite 
places $v_1,\dots,v_r$ and an element of $\brauer(\dR)=\dZ/2$ for each real 
place of $k$.
...
Let's start with some motivation. Let $X$ be an $n$-dimensional smooth 
manifold. Suppose $f:X\to X$ is a smooth map with fixed point $x$. 

connected 
compact manifold, and $f:X\to X$ a smooth map with only finitely many fixed 
points. For any point $x\in X$, one has $\h_n(X,X\setminus x,\dZ) \simeq \dZ$. 
Thus the endomorphism of $\h_n(X,X\setminus x,\dZ)$ induced by $f$ is 
multiplication by some integer, called the \emph{local degree} of $f$ at $x$, 
and written $\deg_x f$. The Lefschetz fixed point theorem is very useful here. 
