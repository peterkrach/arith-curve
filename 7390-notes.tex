...
we know is finite. Unfortunately $\h^1(G_k, A[n])$ is infinite, so we will have 
to work a bit harder. 

(\textbf{what if $k=\mathbb{R}$? What can we say about connected components?
Also, interlude on Kummer exact sequence for $\mathbb{G}_m$, link with 
\'etale cohomology, \'etale fundamental groups})

There is an alternate description of $\h^1\left(G_k,A(\bar k)\right)$. A 
...
of $A$ over $k$. 
\[
  0 \to \operatorname{Sel}_S(M) \to \h^1(G,M) \to \prod_{v\in S} \h^1(G_v,M)
\]
