...
\emph{degree} of $D$ to be $\deg D=\sum D_x \in \mathbb{Z}$. We would like to 
have a definition that works on arbitrary varieties. First, we replace $D$ by 
the invertible sheaf $\mathscr{L}=\sO(D)$, where $\mathscr{L}(U)$ is the ring 
...
degree of any invertible sheaf $\mathscr{L}$ on a projective variety $X/k$. 
Recall that the Euler characteristic of $\mathscr{L}$ is 
$\chi(\mathscr{L}) = \sum (-1)^i h^i(\mathscr{L})$. It is proven in 
\cite[4.1]{dj93} that there exists a (unique) polynomial 
$\phi\in\mathbb{Q}[t]$ such that 
$\chi(\mathscr{L}^{\otimes n}) = \phi(n)$ for $n\gg 0$. If $X$ is a curve, we 
can see this easily; since $\mathscr{L}=\sO(D)$ is effective, the Riemann-Roch 
forces $\chi(\mathscr{L}^{\otimes n}) = n\cdot \deg D + 1 - g$ for $n\gg 0$. 
We define the \emph{degree} of $\mathscr{L}$ to be the coefficient of the 
...
is a disjoint union of ind-varieties \cite[4.8]{kl05}. More precisely, for 
$\phi\in \mathbb{Q}[t]$, let $\pic_X^\phi(T)$ be the set of all 
$\mathscr{L}\in \pic_X(T)$ such that 
$\chi(\mathscr{L}^{\otimes n}) = \phi(n)$ for $n\gg 0$. In other words, 
$\pic_X^\phi$ classifies invertible sheaves with Hilbert polynomial $\phi$. 
It turns out \cite[6.20]{kl05} that for each $\phi$, $\pic_X^\phi$ represents 
a clopen subscheme of $\pic_X$, and $\pic_X$ is covered by the 
$\pic_X^\phi$. Moreover \cite[6.21]{kl05}, $\pic_X^\circ$ is the connected 
component of the identity in $\pic_X$. Just as the genus of a curve is the 
...
One could define $\pic_X^d$ to be the scheme classifying invertible sheaves of 
degree $d$. This works, except that in general $d\in \mathbb{Q}$ rather than 
$d\in \mathbb{N}$. Thus the group 
$\operatorname{NS}(X) = \pic_X(\bar k) / \pic_X^\circ(\bar k)$ -- the 
\emph{N\'eron-Severi group} of $X$, is not necessarily isomorphic to 
$\mathbb{Z}$. On the other hand, if $X=E$ is an elliptic curve, then invertible 
sheaves are classified by their degree, so $\operatorname{NS}(E) = \mathbb{Z}$. 
...
machinery. The moduli stack $\mathcal{M}_g$ which classifies nice curves of 
genus $g$ has dimension $3 g - 3$, while the moduli stack $\mathcal{A}_g$ 
classifying principally polarized abelian varieties of dimension $g$ has 
dimension $\frac{g(g+1)}{2}$. For $g>3$, $\mathcal{A}_g$ has greater 
dimension that $\mathcal{M}_g$, so there are ``more principally polarized 
abelian varieties than jacobians.'' For $g\leqslant 3$, all principally 
