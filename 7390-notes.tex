...
abelian variety, then $G=A(\mathbb{C})$ is a compact, complex Lie group. Let 
$\mathfrak{g}$ be its Lie algebra. Consider the exponential 
$\exp:\mathfrak{g}\to G$\ldots \textbf{look up the rest of this proof}.

The general case in characteristic zero follows from the Lefschetz principle, 
or one can just prove it directly \cite[I.1.4]{mi}. 
...
$C/k$ with $k\subset\mathbb{C}$. For general curves, we have 
...
(\textbf{look up references + include details for general picard scheme, 
albanese scheme})
...
(\textbf{define ``divisor over $L$'' and Picard group more carefully -- work 
this out.})

\begin{remark}
Consider the functor $P_C^\circ:\mathsf{Sch}_k\to\mathsf{Ab}$ that sends 
$T$ to $\pic^\circ(C\times_k T)/\pic(T)$. If $C(k)\ne \varnothing$, then 
$P_C^\circ$ is represented by the jacobian $J$. 
\end{remark}
...
For example, if $C$ is a curve of genus $0$, then $\operatorname{Alb} X=0$. 
(\textbf{prove this}). On the other hand, if the genus $g\geqslant 1$, then 
