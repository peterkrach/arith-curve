...
$D$ by $\mathscr{L}=\sO(D)$, the invertible sheaf induced by $D$. For 
...
$\pic(X)$ is isomorphic to the class group $\Div(X)$ of Cartier divisors (Weil 
divisors if $X$ is a nice curve). Recall that if $\mathscr{M}$ is the sheaf of 
rational functions on $X$, then the group of Cartier divisors is 
$\h^0(X,\mathscr{M}^\times/\sO^\times)$. The short exact sequence 
\[
  1 \to \sO^\times \to \mathscr{M}^\times\to \mathscr{M}^\times/\sO^\times \to 1
\]
induces a long exact sequence in sheaf cohomology:
\[
  0 \to \h^0(\sO^\times) \to \h^0(\mathscr{M}^\times) \to \h^0(\mathscr{M}^\times / \sO^\times) \to \pic(X) \to \h^1(\mathscr{M}^\times) \to \cdots
\]
When $X$ is integral, $\mathscr{M}^\times$ is flasque, so its $\h^1$ is 
trivial, whence $\pic(X)=\Div(X)/\h^0(\sO^\times)$. 

Grothendieck showed that the functor $T\mapsto \pic(X\times T)$ is (after 
fppf-sheafification) representable. The representing scheme is denoted 
$\pic_X$ and called the \emph{Picard scheme} of $X$. We denote by 
$\pic_X^\circ$ the connected component of the identity of $\pic_X$. This is a 
connected group scheme, and if $X$ is a nice variety over a field of 
characteristic zero, then $\pic_X^\circ$ is an abelian variety. Regardless of 
the base characteristic, $\pic_X^\circ$ is abelian variety. For details, see 
\cite{kl05}.

If $X=A$ is already an abelian variety over $k$, then 
$\pic_A^\circ(\bar k) = \pic^\circ(A_{\bar k})$, which turns out to be the 
set $\{c\in \pic(A_{\bar k}) : t_a^* c = c\forall a\in A\}$, where 
$t_a:A\to A$ is translation by $a$. 

(\textbf{Degree for arbitrary sheaves? Is $\pic^\circ$ ``sheaves of degree 
zero'' in general?} It sounds like 
$\pic(A_{\bar k})/\pic^\circ(A_{\bar k})=:NS(A_{\bar k})$, where 
$\operatorname{NS}(A)$ is the Neron-Severi group of $A$.)

(\textbf{Exercise:} make sure this works if $A$ is an elliptic curve!)

In any case, the \emph{dual} of $A$ is defined to be $A^\vee = \pic_A^\circ$.
Choose $\in \pic(A)$; this gives a map $\varphi_c:A\to A^\vee$. At the level of 
sheaves, this is given by $a\mapsto t_a^* c - c$, or equivalently 
$\mathscr{L}\mapsto t_a^*\mathscr{L}\otimes \mathscr{L}^{-1}$. 

(\textbf{Todo:} cite the construction of the map $A\to A^\vee $ given in 
Milne or Mumford). 
...
For $g\geqslant 2$, consider $\mathcal{M}_g$, the moduli space of curves, 
and $\mathcal{A}_g$, the moduli space of principally polarized abelian 
varieties. It turns out that $\dim \mathcal{M}_g = 3 g-3$ and 
$\dim \mathcal{A}_g = \frac{g(g+1)}{2}$. If $g>3$ then 
$g(g+1)/2 > 3 g-3$, so there are abelian varieties that are not principally 
polarized. For $g\leqslant 3$, then all principally polarized abelian varieties 
are jacobians (possibly after a change of the polarization). 

However, all abelian varieties are isogenous to jacobians. (\textbf{source?})
