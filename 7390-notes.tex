...
product, where $n=\operatorname{rk} A$. The group $A(k)/A(k)_\text{tors}$ is a 
...
There is a natural isomorphism $\Lambda\simeq \h_1(G,\dZ)$. One defines this 
by interpreting $\fg^\vee\subset \Omega^1(G)$ as the module of invariant 
differential forms. The pairing $\h_1(G,\dZ)\times \fg^\vee \to \dC$ given by 
\[
  \langle \sigma,\omega\rangle = \int_\sigma \omega
\]
is non-degenerate, so the induced map $\h_1(G,\dZ) \to \fg$ is an injection. To 
see that its image is the kernel of the exponential map, note that $\exp(X)=0$ 
if and only if the flow of $X$ is a loop, which will be in the image of 
$\h_1(G,\dZ)\to \fg$. 
...
isomorphisms $A[n]\simeq \h_1(A(\dC),\dZ/n)$ for all integers $n\geqslant 2$. 
We will think of $A[n]$ as a kind of algebraic avatar for $\h_1(A,\dZ/n)$. 
Unfortunately, there is no algebraic analogue of $\h_1(A,\dZ)$, but there is a 
good substitute. Let $\hat\dZ=\varprojlim \dZ/n$ be the profinite completion of 
$\dZ$. The isomorphisms $A[n]\simeq \h_1(G,\dZ/n)$ are compatible, so we get an 
isomorphism of $\hat\dZ$-modules 
\[
  \hat T A = \varprojlim A[n] \to \varprojlim \h_1(G,\dZ/n) = \h_1(G,\hat\dZ) \text{.}
\]
Since $\hat\dZ=\prod_p \dZ_p$, we will consider $\hat T A$ only one $p$-part at 
a time. This is important because if $A$ is an abelian variety over a field of 
characteristic $p>0$, the groups $A[p^e]$ are not well behaved. 



Now let $A$ be an abelian variety of dimension $d$ over an arbitrary field $k$. 
Recall that $A[n]=\{a\in A(\bar k):n\cdot a=0\}$. As an abelian group, we have 
\[
  A[n]\simeq \begin{cases}
               (\dZ/n)^{\oplus 2 d} & \text{if $n\in k^\times$} \\
               (\dZ/p^e)^{\oplus r} & \text{if $n=p^e$ where $p=\operatorname{char}(k)>0$ and $0\leqslant r\leqslant g$} \\
               (\dZ/p^e)^r\times (\dZ/n')^{\oplus 2d} & \text{if $p=\operatorname{char}(p)\mid n$ and $n=p^e n'$ where $(p,n')=1$}
             \end{cases}
\]

One ought to think of $A[p]$ as a group scheme, namely the fiber product 
$A\times_A 0$ via $A\xrightarrow p A$. As a group scheme, one has 
\[
  A[p] \simeq (\dZ/p\times \mu_p)^r \times \alpha_p^{2(d-r)}
\]
where $\mu_p(R)=\{r\in R:r^p=1\}$ and $\alpha_p(R)=\{r\in R:r^p=0\}$. 

(cite Mumford, add details)
