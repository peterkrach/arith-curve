...
using Kummer theory, one notes that 
$\h^1(\dQ,E[2]) = \left(\dQ^\times/(\dQ^\times)^2\right)^2$. 
\end{example}
...
We will soon prove that $\sel_n(A)$ is finite. Clearly this implies that 
$A(k)/n$ and $\sha(A)[n]$ are finite. 

\begin{question}[Tate-Shafarevich]
If $A$ is an abelian variety over a number field $k$, is $\sha(A)$ finite?
\end{question}
This is currently open. A positive answer would show that for $n\gg 0$, 
$A(k)/n \simeq \sel_n(A)$. In particular, this would imply that for $p$ large 
enough, 
$\operatorname{rk}(A) = \operatorname{rk}_\dZ A(k) = \dim_{\dF_p} \sel_p(A)$. 
...
Let $k$ be a field. An \emph{absolute value} on $k$ is a function 
$|\cdot|:k\to \dR_{\geqslant 0}$ such that 
\begin{enumerate}
  \item $|x|=0$ if and only if $x=0$
  \item $|x y| = |x|\cdot |y|$
  \item $|x+y|\leqslant |x|+|y|$
\end{enumerate}
Given an absolute value $|\cdot |$ on $k$, we can give $k$ the induced 
topology. We call the absolute value with $|x|=1$ for all $x\ne 0$ the 
\emph{trivial absolute value}, and will often tacitly assume that absolute 
values we consider are not trivial. A \emph{place} of $k$ is an equivalence 
class of non-trivial absolute values on $k$, where two absolute values are 
equivalent if they induce the same absolute value. (One can prove this implies 
one is a power of the other.) For a place $v$, let $k_v$ be the completion of 
$k$ at $v$. If $k=\dQ$, then one can classify places of $k$. Either 
$|\cdot|$ is the usual absolute value, which induces the completion $\dR$. Let 
$p$ be a prime. There is a unique homomorphism (the $p$-adic valuation) 
$v_p:\dQ^\times\to \dZ$ with $v_p(p)=1$ and $v_p(\ell) = 0$ for all 
primes $\ell\ne p$. We set $|x|_p = p^{-v_p(x)}$. It turns out that this is an 
absolute value; the completion of $\dQ$ at $|\cdot|_p$ is denoted $\dQ_p$. 

For $k$ an arbitrary number field and $v$ a place of $k$, the completion 
$k_v$ will either be 
\begin{itemize}
  \item $\dR$, with the absolute value induced by an embedding 
    $k\hookrightarrow \dR$
  \item $\dC$, with the absolute value similarly induced. Conjugate embeddings 
    $k\hookrightarrow \dC$ induce the same place.
  \item if $\mathfrak{o}_k$ is the ring of integers of $k$ and 
    $\mathfrak{p}\subset \mathfrak{o}_k$ is some maximal ideal\ldots
\end{itemize}

(\textbf{More stuff on local fields, global fields, Haar measures, the 
``whole package'' in $G_k$\ldots})
