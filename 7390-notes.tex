...
$O\in C(k)$; this gives us an embedding $j:C\hookrightarrow J$, which is an 
isomorphism by the Riemann-Roch theorem. Thus $C$ obtains a group law with $O$ 
as the identity. 

Assume $6$ is a unit in $k$, and begin with an equation $y^2=x^3 + a x+b$ where 
$a,b\in k$ and $4 a^3+27 b^2\ne 0$. Let $C\subset \dP_k^2$ be the zero set of 
\[
  x_1^2 x_2 - x_0^3 - a x_0 x_2^2 - b x_2^3 \text{.}
\]
Then $C$ is a nice curve of genus $1$, and we choose $0=(0:1:0)\in C(k)$. We 
would like an explicit description of the group law on $C$ induced by the 
isomorphism $j:C\to \jac(C)$. We have a rational map $n:C\times C\to C$, where 
$n(x,y)=z$ if $x$ and $y$ are distinct, the unique line $\ell_{x,y}$ passing 
through $x$ and $y$ intersects $C$ transversely, and 
$\ell_{x,y}\cap C=\{x,y,z\}$. It is possible to write down a formula for the 
map $n$, but that will not be necessary here. Let $\alpha:C\to C$ be the 
involution $(x_0:x_1:x_2)\mapsto (x_1:x_0:x_2)$. We will prove that 
$\alpha\circ n$ is the group law on $E$ induced from $\jac(E)$. 

projective closure of the plane curve defined 
...
the trace of Frobenius. Indeed, $a=\operatorname{tr}(\Phi_{E\ast},T_\ell)$ for 
...
\textbf{(use resultants? essentially $\Phi_{A_{\dF_{q^n}}}=\Phi_A^n$)}
...
Since $5$ is a $25$-Weil number, there exists an elliptic curve $E/\dF_{25}$ 
such that $P_E(t)=(t-5)^2$. We will find that $A_{\dF_{25}}$ is isogenous to 
$E\times E$. 
\end{example}
