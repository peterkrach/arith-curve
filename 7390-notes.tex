...
explain everything in the theorem. Recall that $\operatorname{SL}(2,\dR)$ acts 
...
($\Omega_E$ is a real period, the $c_p$ are Tamagawa numbers) There is the 
implicit conjecture that $\sha(E)$ is finite. 
...
Let's explain all these terms. Take a minimal Weierstrass model 
\[
  y^2+a_1 xy+a_3 y=x^3+a_2 x^2+a_4 x+a_6
\]
for $E$, with the $a_i\in \dZ$ and minimal discriminant (with respect to 
divisibility). 

\textbf{(discuss minimal model, clarify definition involving connected 
component of identity)}

We define 
\[
  \Omega_E = \int_{E(\dR)} \frac{dx}{2 y + a_1 x+a_3} \in \dR \text{.}
\]
This is easily computable (using the AG-mean). The integrand is the canonical 
invariant differential on $E$. 

We define the regulator $\operatorname{Reg}_E$. Choose a basis 
$x_1,\dots,x_r$ of $E(\dQ)/E(\dQ)_\text{tors}$. Using our minimal model, we can 
write any $x\in E(\dQ)$ as $x=(x_0:x_1:x_2)$, where the $x_i\in \dZ$ with 
$\gcd(x_0,x_1,x_2)=1$. Recall that the height of $x$ is 
\[
...
\]
(This is just the N\'eron-Tate height associated with our minimal model.) 
This gives us the N\'eron-Tate pairing  
\[
  \langle x,y\rangle = \frac 1 2\left(\widehat h(x+y)-\widehat h(x)-\widehat h(y)\right) \text{.}
\]
This induced a nondegenerate bilinear pairing (hence the structure of a Hilbert 
space) on $V=E(\dQ)\otimes\dR$. There is a canonical Haar measure associated 
with $\langle\cdot,\cdot\rangle$ (\textbf{define this in general}). Let 
$\Lambda$ be the image of $E(\dQ)$ in $V$; this is a lattice, and we set 
\[
  \operatorname{Reg}_E = \operatorname{vol}(V/\Lambda) = \left|\det\left(\langle x_i,x_j\rangle_{i,j}\right) \right|
\]
(the volume of the lattice, maybe to some power). 

Recall that the \emph{Tate-Shafarevich group} $\sha(E)$ is 
\[
  \sha(E) = \ker\left(\h^1(\dQ,E) \to \prod_v \h^1(\dQ_v,E)\right)
\]
where $\h^1(k,E) = \h^1(G_k,E(\bar k))$. 

The $c_p$ are \emph{Tamagawa numbers} of $E$. Let $E_0(\dQ_p)$ be the group of 
points in $E(\dQ_p)$ whose reduction modulo $p$ is is not singular. 
(\textbf{clarify this: $E(\dQ_p)$ should be $E_0(\dQ_p)$ and a formal group})
and we put 
\[
  c_p=[E(\dQ_p):E_0(\dQ_p)]
\]
...
\textbf{(include Tate's quote)}

\begin{example}
Let $E$ be the rational elliptic curve $y^2+x y=x^3+x^2-1154 x-15345$. We 
...
\[
  \frac{\Omega_E \cdot \operatorname{Reg}_E\cdot \prod c_p\cdot \sha(E)}{\# E(\dQ)_\text{tors}^2} = 0.2049\dots \cdot \# \sha(E) = L(E,1) = 1.84\ldots
\]
...
\begin{align*}
  \operatorname{Reg}_E &= 15.36\dots \\
  \Omega_E &= 0.074\ldots \\
  \# E(\dQ)_\text{tors} &= 1 \\
  c_{139} &= 4 \\
...
\[
  \# \sha(E) = \frac{L^{(3)}(E,1)/3!}{\Omega_E \operatorname{Reg}_E \prod c_p / E(\dQ)_\text{tors}^2} = 3.99\dots
\]
