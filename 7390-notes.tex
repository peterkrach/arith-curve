...
\[
  \sum_{\sigma\subset S} (-1)^{|\sigma|} \pi_\sigma^\ast = 0 : \pic(A)\to \pic(A^S) \text{.}
\]
\end{theorem}
\begin{proof}
...
in $\pic(J)$, the \emph{theta-divisor of $J$}. If we set 
$\Theta^-=[-1]^\ast \Theta$ and $\theta^-=[-1]^\ast \theta$, then 
$\langle \cdot,\cdot\rangle_{\theta+\theta^-}$ is positive-semidefinite (see 
\cite[9.4.1]{bg06}) but we will not need this. In general, for a class 
$c\in \pic(A)$ for which $\langle\cdot,\cdot\rangle_c$ is 
positive-semidefinite, set $|\cdot|_c=\widehat h_c^{1/2}$. The following 
theorem is very deep (it implies Faltings' theorem).
...
constants $\lambda,\lambda'$ depending only on $C$ and a chosen 
$x_0\in C(\bar k)$ such that for $x,y\in C\subset J$, if 
...
\end{theorem}
\begin{proof}
This is very difficult -- see \cite[11.9.1]{bg06}.
\end{proof}
