...
desired properties. Choose a very ample divisor $c\in \picard(A)$. This yields 
...
associated with $c$. The Picard group $\picard(A)$ is generated by very ample 
divisors, so we can extend our definition to a map 
$\widehat h:\picard(A)\to \dR^{A(\bar k)}$ given by 
...
is a natural transformation $\widehat h : \picard\to \operatorname{R}$, 
...
$G_k$-module. It is easy to see that $T_\ell$ (and thus $V_\ell$) are actually 
functors from abelian varieties over $k$ to $\dZ_\ell$-representations (or 
$\dQ_\ell$-representations) of $G_k$. 

\begin{theorem}
Let $\ell\in k^\times$ be a prime. For any $\alpha\in \End(A)$, we have 
$P_\alpha = \det(t\cdot 1-\alpha_\ast,T_\ell A)$. 
\end{theorem}

(\textbf{briefly mention crystalline cohomolog, rigid cohomology, source for 
all this.})
...
Essentially $\End^\circ(A) \hookrightarrow \End_{\dZ_\ell}(T_\ell A)$. Use the 
theorem and Cayley-Hamilton. 
\end{proof}

Next time: abelian varieties over finite fields. The polynomial 
$P_{\Phi_A}\in \dZ[t]$ is an important invariant of $A$. It turns out that 
this polynomial determines $A$ up to isogeny. It is possible to characterize 
those polynomials that are of the form $P_{\Phi_A}$. 
