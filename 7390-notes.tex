...
While this is a geometric theorem, it does not have a geometric proof. It 
implies the Mordell conjecture for $k\subset \dC$. Note that this is actually 
a special case of the Mordell-Lang conjecture. The general conjecture is 
much more complicated. 
\todo{Look up KK-trace version, Lang-N\'eron -- Conrad's notes}
...
\section{Overview of proof of Mordell-Weil}

The proof has two main ingredients. The first (the weak Mordell-Weil theorem) 
says that for any integer $n\geqslant 2$, $A(k)/n A(k)$ is a finite group. 

There is a ``height function'' $h:A(k)\to \mathbb{R}_{\geqslant 0}$, such that 
\begin{enumerate}
  \item for any $c>0$, the set $\{h\leqslant c\}=\{x\in A(k):h(x)\leqslant c\}$ 
    is finite
  \item for $x,y\in A(k)$, we have $h(x+y)-h(x-y)=2h(x)+2h(y)$, which implies 
    $h(n\cdot x) = n^2 h(x)$ for all $n\geqslant 1$
\end{enumerate}
One can think of $h$ as a kind of quadratic form: 
``$h(x)=\langle x,x\rangle$.'' There actually is a pairing (\textbf{look this up})

We will show that the weak Mordell-Weil theorem and the existence of a height 
function imply the full Mordell-Weil theorem. By the weak Mordell-Weill 
theorem, there exist $y_1,\dotsc,y_s\in A(k)$ whose images are all of  
$A(k)/n A(k)$. Take any point $x_1\in A(k)$. Then $x_0 = y_{i_1} + n x_2$ for 
some $x_2\in A(k)$. Thus 
\begin{align*}
  h(x_1) &= h(y_{i_1} + n x_2) \\
    &\leqslant 2 h(y_{i_1}) + 2 h(n x_2) \\
    &= 2 h(y_{i_1}) + 2 n^2 h(x_2) 
\end{align*}
We had $h(n x_2) = h(x_1-y_{i_1})$, and 
$n^2 h(x_2) \leqslant 2 h(x_1) + 2 h(-y_{i_1})$, so 
$h(x_2) \leqslant \frac{2}{n^2} h(x_1) + \frac{2}{n^2} h(-y_{i_1})$. Let 
$C=2 \sup\{h(y_i)\}$. Then 
$h(x_1) \leqslant \frac{2}{n^2} h(x_1) + \frac{C}{n^2}$. If 
$h(x_1)\geqslant C$, then 
$h(x_2)\leqslant \frac{2}{n^2} h(x_1) + \frac{1}{n^2} h(x_1) = \frac{3}{n^2} h(x_1)$. 
Since we have $n\geqslant 2$, this yields $h(x_2) < h(x_1)$. We can continue 
this process to obtain $x_1\equiv x\mod \langle y_1,\dotsc,y_s\rangle$ for some 
$x=x_t$ with $h(x)\leqslant C$. Since $h(y_i)\leqslant C$ for each $i$, we see 
that $A(k)$ is generated by the finite set $\{h\leqslant C\}$.

\todo{Check details + clean up notation}

Proofs of this form are called proofs \emph{by descent}. Our proof is (very 
nearly) effective; given a set of generators for $A(k)/n A(k)$, it gives an 
algorithm for finding a set of generators for $A(k)$. Moreover, one can choose 
any $n$ -- most people use $n = 2$ when doing computations. In the next 
section, we will give a proof of the weak Mordell-Weil theorem using Galois 
cohomology. Let $\bar k$ be the algebraic closure of $k$, and let  
$G_k = \gal(\bar k/k)$ be the absolute Galois group of $k$. The abelian group 
$A(\bar k)$ has a continuous $G_k$-action. One way to see this is to embed 
$A$ into some $\mathbb{P}^N$, and let $G_k$ act on coordinates. A fancier 
way to see this is to note that $A(\bar k) = \hom(\spec(\bar k),A)$, so if 
$\sigma\in G_k$, $x\in A(\bar k)$, the point $^\sigma x$ is the composite 
...
We are interested in continuing this exact sequence to the right -- we'll 
get a long exact sequence 
\[
  0 \to A[n]^{G_k} \to A(k) \to A(k) \to \h^1(G_k,A[n]) \to \h^1(G_k,A(\bar k))\to \h^1(G_k, A(\bar k)) \to \h^2(G_k, A[n]) \to \cdots
\]
