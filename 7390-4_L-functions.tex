...
An easy example is $d=2$.  One has $C(2) = 2$, $D(2) = 2$, so 
...
$\varphi_p(E) = a_p(E) p^{-1/2} /2$, the numbers $\varphi_p(E)$ lie in the 
interval $[-1,1]$. It is natural to ask, for fixed $E$, how the $\varphi_p(E)$ 
...
it is either a theorem or a definition that the dual of $C_0(X)$ is isomorphic 
as a Banach space to the space of Borel measures on $X$. A measure $\mu$ 
corresponds to the functional 
\[
  f\mapsto \mu(f) = \int_X f\, d\mu \text{.}
\]
...
\]
We will often identify $\omega$ with the measure induces. In particular, we 
...
$\varphi_p(E) = a_p(E) / 2\sqrt p$. The Sato-Tate conjecture predicts the 
distribution of the $\varphi_p$ in the interval $[-1,1]$. If $E$ has complex 
...
exactly when $\left(\frac{-D}{p}\right) = -1$. This 
...
sequence $\{\varphi_p(E)\}_p$ is equidistributed in $[-1,1]$ with respect to 
the  measure 
\[
...
$\varphi_p(E)$ are distributed according to the following graph, corresponding 
...
$\{\varphi_p(E)\}_p$ is equidistributed in $[-1,1]$ with respect to the 
measure 
\[
...
\subsection{The refined Sato-Tate conjecture}
