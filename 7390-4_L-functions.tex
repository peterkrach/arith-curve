...
In the paper \cite{go85}, Goldfeld, drawing on work of Gross and Zagier 
...
effectively computable constant $c_\varepsilon>0$ such that the class number 
$h(D)$ of the field $\dQ(\sqrt{-D})$ for $D>0$ square-free satisfies the bound 
\[
  h(D) > c_\varepsilon \left(\log D\right)^{1-\varepsilon} \text{.}
\]
It follows that for \emph{any} constant $C$, it is possible to enumerate the 
(finite) list of imaginary quadratic fields with class number $\leqslant C$. 
