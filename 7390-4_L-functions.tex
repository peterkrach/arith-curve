...
\[
  \frac{2}{\pi} \sqrt{1-t^2}\, dt \text{.}
\]
This has a natural reformulation which makes generalization easy. The 
...
$X$ for the space $\operatorname{Cl}\left(\operatorname{SU}(2)\right)$ of 
...
measure $\mu$. Let $\pi:K \to X$ be the canonical projection. 

\begin{theorem}
If $E$ is an elliptic curve with complex multiplication, then the 
set $\{\varphi_p(E)\}\subset \operatorname{Cl}(\operatorname{SU}(2))$ is 
equidistributed with respect to the measure $\pi_\ast \mu$, where $\mu$ is the 
normalized Haar measure on $N(\operatorname{U}(1))$. 
\end{theorem}
\begin{proof}
This is a restatement of Theorem \ref{thm:deuring-hecke}. To see this, note 
that the trace induces an isomorphism 
\[\xymatrix{
  \trace : \operatorname{Cl}\left(\operatorname{SU}(2)\right) \ar[r]^-\sim 
    & [-2,2] \text{.}
}\]
...
For non-CM elliptic curves, let $K=\operatorname{SU}(2)$, and write 
$\pi:K \to X$ for the canonical projection. If $\mu$ is a normalized Haar 
measure on $K$, then the Sato-Tate conjecture states that 
$\{\varphi_p\}\subset X$ is uniformly distributed with respect to the measure 
$\pi_\ast \mu$. To see this, check that 
the pushforward by the trace of the normalized Haar measure on $K$ to $[-2,2]$ 
is $\frac{1}{2\pi} \sqrt{4-t^2} dt$. 

More generally, let $A$ be a $d$-dimenaional abelian variety over $\dQ$, and 
...
absolute value $1$. Thus $\varphi_p(A)$ determines a conjugacy class in 
$\operatorname{SU}(2 d,\dC)$. Set 
$X=\operatorname{Cl}\operatorname{SU}(2 d,\dC)$ and write 
$\pi:\operatorname{SU}(2d,\dC) \to X$ for the canonical projection. As before, 
we can think of $\varphi=\varphi(A)$ as a map 
$\{\text{good primes}\} \to X$. 


\begin{conjecture}[Serre]
There exists a compact real Lie group $K$ in $\operatorname{SU}(2g,\dC)$ such 
that $\{\varphi_p(A)\}\subset X$ is equidistributed with respect to 
$\pi_\ast \mu$, where $\mu$ is the normalized Haar measure on $K$.
\end{conjecture}

One can try to predict the group $K$. It is possible to do this conjecturally 
in much greater generality. The following mostly follows Serre's original 
paper \cite{se94}, but see \cite{se12} for a more elementary and explicit 
approach. 
...
Let $k$ be a field, and let $X$ be a smooth variety over $k$. Write 
...
degree map $\deg:\chow^{\dim X} \to \dZ$, and we say a cycle 
$\alpha \in \chow^r(X)$ is \emph{numerically equivalent to zero} if 
$\deg(\alpha\cdot \beta)=0$ for all $\beta\in \chow^{\dim X-r}(X)$. Write 
$\chow_\text{num}(X)$ for the quotient of the (graded) ideal in $\chow(X)$ 
generated by 
\[
  \{\alpha\in \chow(X) : \alpha\text{ is numerically eqivalent to zero}\} \text{.}
\]

\begin{definition}
Let $k$ be a field. A \emph{(pure) motive over $k$} is a triple 
$(X,e,m)$, where $X$ is a smooth projective variety over $k$, 
$e\in \chow_\textnormal{num}(X\times X)_\dQ$ is an idempotent, and 
$m\in \dZ$. 
\end{definition}

See \cite[4.1.3]{an04} for details. One defines a morphism 
$(X,e,m) \to (Y,f,n)$ to be an element of 
\[
  f\cdot \chow^{\dim X - m - n}(X\times Y)_\dQ\cdot e \text{.}
\]
...
has a Tannakian structure via $h(X)\otimes h(Y) = h(X\times Y)$. Much more 
strikingly, $\motives(k)$ is a semisimple abelian category \cite{ja92}. One 
should think of a Weil cohomology theory as a functor 
$\h:\motives(k) \to \mathsf{grAlg}_L$ for some field $L$. 

\textbf{(add some details, better perspective)}
...
$M = (X,e,m)$ the vector space 
\[
  \h_\sigma(M) = \h_\text{sing}^\bullet(X(k\xrightarrow\sigma\dC), \dQ)
\]
...
\[
  G_\text{mot}(k) = \aut^\otimes(\h_\sigma) \text{.}
\]
There are obvious projections from $G_\text{mot}(k)$ to the groups 
$G_M$
...
real (resp. complex) places of $k$. Let $h$ be the class number of $k$, $d_k$ 
be the discriminant of $k$. Then 
\[
  \lim_{s \to 1} \frac{\zeta_k(s)}{(s-1)^r} = \frac{2^{r_1} (2\pi)^{r_2} h \regulator_k}{\# \mu(k) |d_k|^{1/2}} \text{.}
\]
