...
sake of space, write $A(k)/n$ instead of $A(k)/n A(k)$. 
...
We can define $\h^\bullet(G_k,A[n])$ using the formalism of derived functors. Consider the 
...
$\h^1(\dQ,E[2]) = (\dQ^\times/2)^{\oplus 2}$, which is $\dF_2$-vector space of 
dimension $\aleph_0$. 
\end{example}
