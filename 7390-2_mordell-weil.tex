...
field, often a \emph{number field}, i.e. $k$ is a finite extension of 
$\dQ$. Important examples are $k=\dQ(\sqrt d)$ and $\dQ(\zeta_{p^n})$. 
...
To prove the weak Mordell-Weil theorem, we will use a lot of group cohomology. 
For some motivation, 
...
Here is a very brief explanation of the construction of $|\cdot|$. 
There is a canonical height function on $\dP^n(\bar k)$ defined by  
\[
...
\begin{tikzcd}
  0 \rar 
    & {M'}^G \rar{f}
    & M^G \rar{g}
    & {M''}^G \ar[out=355,in=175]{dll}{\delta} \\
  & \h^1(G,M') \rar{f_\ast}
    &\h^1(G,M) \rar{g_\ast} 
    & \h^1(G,M'') \text{.}
\end{tikzcd}
...
\subsection{Restricted ramification}
...
\end{theorem}
\begin{proof}
For $\# S=3$ this is just \cite[2.7]{gm13}. %\cite[III.10]{mu08}. 
