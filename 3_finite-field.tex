...
Let $\fg=\lie(G)$ be the Lie algebra of $G$. Since $G$ is commutative, the 
exponential map $\exp:\fg\to G$ is a group homomorphism with open image. Since 
$G$ is connected, the image of $\exp:\fg\to G$ is all of $G$, and so we have an 
exact sequence 
\[\xymatrix{
  0 \ar[r] 
    & \Lambda \ar[r] 
...
\[
  \End(G) \to \End_\dQ{\h_1(G,\dQ)} \simeq \mat_{2 d}(\dQ) .
\]
...
completion of $\dZ$. The isomorphisms $\torsion n G\simeq \h_1(G,\dZ/n)$ are 
compatible, so we get an isomorphism of $\widehat\dZ$-modules 
\[
  \tate G := \varprojlim \torsion n G \isomorphism \varprojlim \h_1(G,\dZ/n) = \h_1(G,\widehat\dZ) .
\]
...
\[
   \torsion p A\simeq (\dZ/p)^r\times \boldsymbol\alpha_p^{2(d-r)}\times \boldsymbol\mu_p^r .
\]
Here $\boldsymbol\alpha_p(R)=\{r\in R:r^p=0\}$ and 
$\boldsymbol\mu_p(R)=\{r\in R:r^p=1\}$ for any 
$\dF_p$-algebra $R$. One might hope that $\torsion{p^e}{A}$ can be described just as 
easily for $e>1$, but this is not the case. The group schemes $\torsion{p^e}{A}$ 
exhibit 
very complicated behavior as $e\to \infty$. Indeed, their ``formal inductive 
limit'' $\torsion{p^\infty}{A}$, called the \emph{Barsotti-Tate group} of $A$, 
carries quite a lot of information about $A$. 
...
\begin{definition}\label{def:tate-module}
Let $A$ be an abelian variety. The \emph{$\ell$-adic Tate module of $A$} is 
\[
  \tate_\ell A = \varprojlim \torsion{\ell^n}{A} = \{(a_1,a_2,\dots) : a_n\in \torsion{\ell^n}{A}\text{ and }\ell x_{n+1}=x_n\} .
\]
\end{definition}
...
\[
  \rho_{A,\ell}:G_k\to \operatorname{GL}(\tate_\ell A)\simeq \gl{2 d}{\dZ_\ell} .
\]
The action of $\End(A)$ and $G_k$ commute, so we actually have a 
representation $\End(A)[G_k] \to \mat_{2 d}(\dZ_\ell)$. We could also write this 
...
\[
  \pi_1(A) \simeq G_k \ltimes_\rho \tate A .
\]
...
Let $\torsion{\ell^\infty}{A} = \bigcup_{n\geqslant 1} \torsion{\ell^n}{A}$. As an 
abelian group, $\torsion{\ell^\infty}{A}\simeq \torsion{\ell^\infty}{(\dQ/\dZ)}$, 
which we denote 
...
$\aut \torsion{\ell^\infty}{A}\simeq \gl{2 d}{\dZ_\ell}$. The induced 
...
image of $f$ is contained in the (discrete) subvariety $\torsion n A\subset A$. Since 
$f(A)$ is irreducible, we have $f=0$. Thus $\End A$ is torsion-free, so it 
embeds into the ring
\[
  \End^\circ A = \End_{\mathsf{AbVar}_k^\mathrm{iso}}(A) = \End(A)\otimes\dQ .
\]
...
\[
  \End^\circ(A) \simeq \prod_{i=1}^r M_{e_i}\left(D_i\right) .
\]
...
$\End^\circ(A)\simeq \mat_2\left(\End^\circ(E)\right)$. 
\end{example}
...
$\geomfrob_X=\geomfrob_{X,q}:X\to X$ for the map that on projective coordinates is 
\[
...
$(-1,-p)$, i.e.  
\[
  D = \dQ\left\langle i,j,k : i^2=-1,j^2=-p,i j=k, j i=-k\right\rangle .
\]
\end{example}
...
homomorphism $D\to \End(V)\simeq \mat_2(\dQ)$. Since both rings have the same 
dimension, this would yield $D\simeq \mat_2(\dQ)$. But $\mat_2(\dQ)$ has 
...
that $\torsion n A = \ker[n]\simeq (\dZ/n)^{\oplus 2 d}$, at least if 
$n\in k^\times$. 
\end{example}
...
\[
  \deg\left(c_1 e_1 + \cdots + c_r e_r\right) = f(c_1,\dots,c_r) .
\]
...
degree map $\deg:k\to \dQ$ is the classical norm map $\norm_{k/\dQ}$.
...
\[
  \End(A) = \{\phi\in \End_\dC(\dC^d) : \phi(\Lambda)\subset \Lambda\} .
\]
...
\[
  \deg(\alpha) 
    ...
    = \det\left(\alpha_\ast,\h_1(T,\dZ)\right) .
\]
Since $\deg(n-\alpha)=\det(n\cdot 1-\alpha_\ast,\h_1(T,\dZ))$ for all $n$, we 
conclude that$P_\alpha=\det\left(t\cdot 1-\alpha_\ast,\h_1(T,\dZ)\right)$. 
\end{example}

Recall the Tate module $\tate_\ell A$ as in \autoref{def:tate-module}. To make 
our analogy between $\tate_\ell A$ and $\h_1(A,\dA_\ell)$ precise, we have the 
following theorem: 

\begin{theorem}
There is a natural isomorphism $\tate_\ell A = \h^1(\et A,\dZ_\ell)^\vee$. 
\end{theorem}
\begin{proof}
This is essentially a tautology. By \cite[II.2.4]{de77}, for lisse $\ell$-adic 
sheaves 
$\sF$ on $A$ one has 
$\h^\bullet(A,\sF) = \h^\bullet\left(\pi_1(A),\sF_0\right)$, where $\sF_0$ is 
the stalk of $\sF$ at $0$. By \cite[10.37]{gm13}, we know that 
$\pi_1(A)=\tate(A)$. For $\sF=\dZ_\ell$, this gives 
...
\end{corollary}
\begin{proof}
We use the fact \cite[I.10.15]{mi-av} that the natural map 
\[
  \End(A)\otimes\dZ_\ell \to \End_{\dZ_\ell} \left(\tate_\ell A\right)
\]
is injective. In $\End(\tate_\ell A)$, we have $P_\alpha(\alpha)=0$ by the 
...
we have to work harder. Of course, $\tate_p A$ is a $\dZ_p$-module with a 
...
\[\xymatrix{
  0 \ar[r] 
    & C \ar[r] 
    & \torsion{\ell^e}{A} \ar[r]^-f 
    & \torsion{\ell^e}{A} \ar[r]
    & 0 ,
}\]
where $C$ is independent of $e$ if $e\gg 1$ (since $C\subset \ker(f)$, a finite 
group). At the level of Tate modules, if $f_\ast(x)=0$, then 
$f(x_i)=0$ in $\torsion{\ell^i}{B}$ for all $i$, so each $x_i\in C$. But $C$ is a finite 
...
It turns out that $P_A(0)=\deg(-\geomfrob_A)=\deg(\geomfrob_A)=q^d$; this can be verified 
...
the trace of Frobenius. Indeed, $a=\trace(\geomfrob_{E,\ast},\tate_\ell A)$ for 
...
\[
  \left|\frac{a\pm \sqrt{a^2-4 q}}{2}\right| 
    ...
    = \sqrt q .
\]
If $\dF_q$ has characteristic $\geqslant 5$, we can write $E$ as 
$y^2=x^3+a x+b$. It follows that 
\[
  \# E(\dF_q) = q+1 + \sum_{x\in \dF_q} \left(\frac{x^3+a x+b}{q}\right) ,
\]
